...
| **Evolução Delivery — mensagens por janela (UI)** | `conf_delivery_insights_timeseries.csv` | Não usar `conf_delivery_insights_4h.csv` para presets 5m/15m/1h/hoje/ontem (snapshot fixo ~4h); sem timeseries, mostrar só **segmentos** a partir do horário. CI: **matrix 8 contas em paralelo** + `scripts/merge_delivery_insights_timeseries.py` — tempo de parede ≈ o da **conta mais pesada**. **Limite:** `DELIVERY_INSIGHTS_TS_MAX_PAGES` (default **2500** ≈ **2,5M** msgs/conta/run); incremental entre runs não há (API REST re-lê mensagens por janela). |
