...
| **Evolução Delivery — mensagens por janela (UI)** | `conf_delivery_insights_timeseries.csv` | Não usar `conf_delivery_insights_4h.csv` para presets 5m/15m/1h/hoje/ontem (snapshot fixo ~4h); sem timeseries, mostrar só **segmentos** a partir do horário. **Limite:** o script pagina `Messages.json` com teto `DELIVERY_INSIGHTS_TS_MAX_PAGES` (default **1000** ≈ 1M msgs/conta/run); contas com **>~1M msgs** na janela `DELIVERY_INSIGHTS_TS_HOURS` ficam **abaixo** do Messaging Insights da consola (agregado sem teto). Aumentar `MAX_PAGES` / reduzir horas / `TEST_INSIGHTS_TS_ACCOUNT` para uma conta |
